...
export const fetchFiveDaysWeather = async (city,country,lang) => {
    try {
        ...
        const url_weather = `http://api.openweathermap.org/data/2.5/forecast?lat=${data_coords[0].lat}&lon=${data_coords[0].lon}&appid=${import.meta.env.VITE_APP_ID}&lang=${lang}`;
        const response_weather = await fetch(url_weather);
        const data_weather = await response_weather.json();
